...
if __name__ == '__main__':
    ...
    for i, survey in enumerate(cos_survey):
        ...
        sim_sf_cfrac, sim_sf_err = sim_binned_cfrac(sim_dict, (sim_dict['ssfr'] > quench), sim_dict['dist_bins_sf'], det_thresh[i], lines[i], boxsize)
        sim_q_cfrac, sim_q_err = sim_binned_cfrac(sim_dict, (sim_dict['ssfr'] < quench), sim_dict['dist_bins_q'], det_thresh[i], lines[i], boxsize)
        ...
        cos_sf_cfrac = cos_binned_cfrac(cos_dict, (cos_dict['ssfr'] > quench), cos_dict['dist_bins_sf'], det_thresh[i])
        cos_q_cfrac = cos_binned_cfrac(cos_dict, (cos_dict['ssfr'] < quench), cos_dict['dist_bins_q'], det_thresh[i])
        ...
        c1 = ax[i].errorbar(cos_dict['plot_bins_sf'], cos_sf_cfrac, xerr=cos_sf_xerr, 
                                capsize=4, c='c', marker='', ls='', label=label+' SF')
        c2 = ax[i].errorbar(cos_dict['plot_bins_q'], cos_q_cfrac, xerr=cos_q_xerr, 
                                capsize=4, c='m', marker='', ls='', label=label+' Q')
        leg1 = ax[i].legend([c1, c2], [label+' SF', label+' Q'], fontsize=10.5, loc=1)
        
        l1 = ax[i].errorbar(sim_dict['plot_bins_sf'], sim_sf_cfrac, yerr=sim_sf_err, capsize=4, c='b', marker='o', ls='--')
        l2 = ax[i].errorbar(sim_dict['plot_bins_q'], sim_q_cfrac, yerr=sim_q_err, capsize=4, c='r', marker='o', ls='--')
        if i == 0:
