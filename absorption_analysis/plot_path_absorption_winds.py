...
if __name__ == '__main__':
    ...
    for i, survey in enumerate(cos_survey):
        ...
        for j, w in enumerate(winds):
            ...
            ax[i].plot(plot_bins_sim_sf, sim_sf_path_abs, linestyle=ls[j], c='b', )
            ax[i].plot(plot_bins_sim_q, sim_q_path_abs, linestyle=ls[j], c='r', )
            if j == 0:
                ax[i].fill_between(plot_bins_sim_sf, sim_sf_path_abs - sim_sf_err, sim_sf_path_abs + sim_sf_err, linestyle=ls[j], color='b', alpha=0.1)
                ax[i].fill_between(plot_bins_sim_q, sim_q_path_abs - sim_q_err, sim_q_path_abs + sim_q_err, linestyle=ls[j], color='r', alpha=0.1)
        ...
        if compare:
            ...
            c1, = ax[i].plot(plot_bins_cos_sf, np.log10(cos_sf_path_abs), c='c', marker='o', ls='--', label=label+' SF')
            c2, = ax[i].plot(plot_bins_cos_q, np.log10(cos_q_path_abs), c='m', marker='o', ls='--', label=label+' Q')
