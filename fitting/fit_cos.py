...
import numpy as np
import sys
sys.path.append('/disk2/sapple/VAMP')
...
data_dir = '/disk2/sapple/cgm/cos_samples/test/'
...
for cos_gal in cos_gals:
	...
	for i in range(len(gal_ids)):
		...
		vmin = vgal[i] - vel_range
		vmax = vgal[i] + vel_range

		for d in direction:
			filename = data_dir+'spectra/cos_galaxy_'+str(cos_gal)+'_sample_galaxy_'+str(gal_ids[i])+'_'+d+'.h5'
			with h5py.File(filename, 'r') as f:
				waves = f['wavelength'].value
				vels = f['velocity'].value
				#noise = f['noise'].value
				fake_noise = np.array([0.01]*len(waves))
			f.close()
			
			mask = (vels < vmax) & (vels > vmin)

			waves_use = waves[mask]
			noise_use = waves[mask]

			for line in lines:
				with h5py.File(filename, 'r') as f:
					taus = f[line+'_tau'].value
				f.close()

				taus_use = taus[mask]

				plotname = files[0].split('/', -1)[-1][:-3]
				params, flux_model = fit_spectrum(waves_use, noise_use, taus_use, voigt=False, folder=plot_dir+name)
		...
		with h5py.File(output+'spectra/cos_galaxy_'+str(cos_gal)+'_sample_galaxy_'+str(gal_ids[i]), 'a') as p:
