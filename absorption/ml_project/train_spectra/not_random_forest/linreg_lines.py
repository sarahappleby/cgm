...
if __name__ == '__main__':
    ...
    # Step 5) Run and save the linear regression routine
    reg = LinearRegression()
    reg.fit(feature_scaler.transform(df_full[train][features]), predictor_scaler.transform(df_full[train][predictors]))
    pickle.dump([reg, features, predictors, feature_scaler, predictor_scaler, df_full], 
                open(f'{model_dir}{model}_{wind}_{snap}_{lines_short[lines.index(line)]}_lines_LR.model', 'wb'))

    # Step 6) Predict conditions
    conditions_pred = pd.DataFrame(predictor_scaler.inverse_transform(reg.predict(feature_scaler.transform(df_full[~train][features]))),columns=predictors)
