...
def generate_pygad_spectrum(s, los, line, lambda_rest, vbox, periodic_vel, v_limits, Nbins, snr, spec_name, save_dir):
    ...
    with h5py.File(f'{save_dir}{spec_name}.h5', 'a') as hf:
        ...
        hf.create_dataset(line+'_flux_conv', data=np.array(f_conv))
        hf.create_dataset(line+'_flux_conv_noise', data=np.array(f_conv_noise))
        hf.create_dataset(line+'_flux_conv_noise_contin', data=np.array(f_conv_noise_contin))
