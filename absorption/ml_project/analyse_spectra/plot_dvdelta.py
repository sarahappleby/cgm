...
if __name__ == '__main__':
    ...
    for l, line in enumerate(lines):
        ...
        for i in range(len(fr200)):
            ...
            with h5py.File(results_file, 'r') as hf:
                ...
                dv.extend(hf[f'pos_dv_{fr200[i]}r200'][:])
        ...
        plot_order = np.arange(len(dv[sf_mask]))
        np.random.shuffle(plot_order)
        im = ax[l][0].scatter(dv[sf_mask][plot_order], delta_rho[sf_mask][plot_order], c=all_r[sf_mask][plot_order], cmap=cmap, norm=norm, s=1, alpha=0.6)
        bin_cent,ymean,ysiglo,ysighi,ndata = pm.runningmedian(dv[sf_mask*inner_mask*silly_mask], delta_rho[sf_mask*inner_mask*silly_mask], stat='median')
        ax[l][0].plot(bin_cent[ndata>nmin], ymean[ndata>nmin], c='mediumorchid', lw=1.5, ls='--', path_effects=[outline])
        bin_cent,ymean,ysiglo,ysighi,ndata = pm.runningmedian(dv[sf_mask*~inner_mask*silly_mask], delta_rho[sf_mask*~inner_mask*silly_mask], stat='median')
        ax[l][0].plot(bin_cent[ndata>nmin], ymean[ndata>nmin], c='mediumorchid', lw=1.5, ls='-.', path_effects=[outline])
        
        plot_order = np.arange(len(dv[gv_mask]))
        np.random.shuffle(plot_order)
        im = ax[l][1].scatter(dv[gv_mask][plot_order], delta_rho[gv_mask][plot_order], c=all_r[gv_mask][plot_order], cmap=cmap, norm=norm, s=1, alpha=0.6)
        bin_cent,ymean,ysiglo,ysighi,ndata = pm.runningmedian(dv[gv_mask*inner_mask*silly_mask], delta_rho[gv_mask*inner_mask*silly_mask], stat='median')
        ax[l][1].plot(bin_cent[ndata>nmin], ymean[ndata>nmin], c='mediumorchid', lw=1.5, ls='--', path_effects=[outline])
        bin_cent,ymean,ysiglo,ysighi,ndata = pm.runningmedian(dv[gv_mask*~inner_mask*silly_mask], delta_rho[gv_mask*~inner_mask*silly_mask], stat='median')
        ax[l][1].plot(bin_cent[ndata>nmin], ymean[ndata>nmin], c='mediumorchid', lw=1.5, ls='-.', path_effects=[outline])

        plot_order = np.arange(len(dv[q_mask]))
        np.random.shuffle(plot_order)
        im = ax[l][2].scatter(dv[q_mask][plot_order], delta_rho[q_mask][plot_order], c=all_r[q_mask][plot_order], cmap=cmap, norm=norm, s=1, alpha=0.6)
        bin_cent,ymean,ysiglo,ysighi,ndata = pm.runningmedian(dv[q_mask*inner_mask*silly_mask], delta_rho[q_mask*inner_mask*silly_mask], stat='median')
        ...
        if l == len(lines)-1:
            for i in range(3):
                ax[l][i].set_xlabel(r'${\rm log} (\delta v /{\rm km s}^{-1})$')
    ...
    plt.savefig(f'{plot_dir}{model}_{wind}_{snap}_dvdelta_r200.png', format='png')
    plt.close()
