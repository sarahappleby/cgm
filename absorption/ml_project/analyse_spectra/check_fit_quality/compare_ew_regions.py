...
if __name__ == '__main__':
    ...
    for line in lines:
        ...
        for k in range(len(fr200)):
            ...
            with h5py.File(results_file, 'r') as hf:
                all_ewspec.extend(hf[f'ewspec_{fr200[k]}r200'][:])
                all_ewfit.extend(hf[f'ewfit_{fr200[k]}r200'][:])
                all_chisq.extend(hf[f'chisq_{fr200[k]}r200'][:])
                all_fr200.extend([fr200[k]] * len(hf[f'chisq_{fr200[k]}r200'][:]))

        all_ewspec = np.array(all_ewspec)
        all_ewfit = np.array(all_ewfit)
        all_chisq = np.array(all_chisq)
        all_fr200 = np.array(all_fr200)

        im = ax[i][j].scatter(all_ewspec, all_ewfit, c=np.log10(all_chisq + 1e-2), s=1, cmap=cmap, vmin=-1, vmax=2)
        ...
        ax[i][j].annotate(plot_lines[lines.index(line)], xy=(0.05, 0.91), xycoords='axes fraction',
    ...
    plt.savefig(f'{plot_dir}{model}_{wind}_{snap}_compare_ew_fit_regions.png')
    plt.clf()
