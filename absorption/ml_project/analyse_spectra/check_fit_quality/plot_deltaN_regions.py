...
if __name__ == '__main__':
    ...
    for line in lines:
        ...
        all_deltaN = all_Nfit - all_Nspec

        im = ax[i][j].scatter(all_Nspec, all_deltaN, c=np.log10(all_chisq + 1e-2), s=1, cmap=cmap, vmin=-1, vmax=2)
        ax[i][j].axhline(0, c='k', ls='--', lw=1)

        if line in ['H1215', "SiIII1206"]:
            ax[i][j].set_ylabel(r'${\rm log} (N_{\rm fit} / N_{\rm sum})$')
        if line in ["SiIII1206", "CIV1548", "OVI1031"]:
            ax[i][j].set_xlabel(r'${\rm log (N}/{\rm cm}^{-2})_{\rm sum}$')
        ...
        if line == 'SiIII1206':
            ax[i][j].set_yticks(np.arange(-4, 3, 2))
    ...
    plt.savefig(f'{plot_dir}{model}_{wind}_{snap}_compare_deltaN_fit_lines.png')
    plt.clf()
