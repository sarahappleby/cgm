...
if __name__ == '__main__':
    ...
    for l, line in enumerate(lines):
        ...
        for i in range(len(fr200)):
            ...
            with h5py.File(results_file, 'r') as hf:
                ...
                Z.extend(hf[f'log_Z_{fr200[i]}r200'][:])

        N = np.array(N)
        rho = np.array(rho)
        chisq = np.array(chisq)
        ids = np.array(ids)
        Z = np.array(Z) - np.log10(zsolar[l])
        ...
        im = ax[l][0].scatter(N[sf_mask][plot_order], delta_rho[sf_mask][plot_order], c=Z[sf_mask][plot_order], cmap=cmap, s=1, alpha=0.6)
        bin_cent,ymean,ysiglo,ysighi,ndata = pm.runningmedian(N[sf_mask*silly_mask], delta_rho[sf_mask*silly_mask], stat='median')
        ...
        np.random.shuffle(plot_order)
        im = ax[l][1].scatter(N[gv_mask][plot_order], delta_rho[gv_mask][plot_order], c=Z[gv_mask][plot_order], cmap=cmap, s=1, alpha=0.6)
        bin_cent,ymean,ysiglo,ysighi,ndata = pm.runningmedian(N[gv_mask*silly_mask], delta_rho[gv_mask*silly_mask], stat='median')
        ...
        np.random.shuffle(plot_order)
        im = ax[l][2].scatter(N[q_mask][plot_order], delta_rho[q_mask][plot_order], c=Z[q_mask][plot_order], cmap=cmap, s=1, alpha=0.6)
        bin_cent,ymean,ysiglo,ysighi,ndata = pm.runningmedian(N[q_mask*silly_mask], delta_rho[q_mask*silly_mask], stat='median')
    ...
    fig.colorbar(im, cax=cbar_ax,label=r'${\rm log} (Z / Z_{\odot})$')
    fig.subplots_adjust(wspace=0., hspace=0.)
    plt.savefig(f'{plot_dir}{model}_{wind}_{snap}_Ndelta_Z.png')
    plt.close()
