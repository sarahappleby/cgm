...
if __name__ == '__main__':
    ...
    for l, line in enumerate(lines):
        ...
        profile_file = f'{results_dir}{model}_{wind}_{snap}_{line}_{log_frad}log_frad_median_ew_profile.h5'

        if os.path.isfile(profile_file):
            plot_data = read_h5_into_dict(profile_file)
        else:

            ew_dict = read_h5_into_dict(f'{results_dir}{model}_{wind}_{snap}_{log_frad}log_frad_ew_{line}.h5')
    ...
    plt.savefig(f'{plot_dir}{model}_{wind}_{snap}_sat_test_ew_profile_{log_frad}log_frad.png')
    plt.show()
    plt.clf()
