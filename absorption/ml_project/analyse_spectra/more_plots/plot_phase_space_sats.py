...
if __name__ == '__main__':
    ...
    for l, line in enumerate(lines):
        ...
        if l in [0, 1]:
            ax[l].set_xticks(range(-1, 5))
        elif l == 2:
            ax[l].set_xticks(range(-1, 6))

    ax[0].set_ylabel(r'${\rm log } (T / {\rm K})$')

    fig.subplots_adjust(wspace=0., hspace=0.)
    plt.savefig(f'{plot_dir}{model}_{wind}_{snap}_sats_deltaTN_{lines[0]}_{lines[1]}_{lines[2]}_chisqion_all_r200.png')
    plt.close()
