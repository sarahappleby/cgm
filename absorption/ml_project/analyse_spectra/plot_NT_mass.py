...
if __name__ == '__main__':
    ...
    for l, line in enumerate(lines):
        ...
        for i in range(len(mass_plot_titles)):
            ...
            im = ax[l][i].scatter(N[mass_mask][plot_order], T[mass_mask][plot_order], c=all_r[mass_mask][plot_order], cmap=cmap, norm=norm, s=1.5)
            bin_cent,ymean,ysiglo,ysighi,ndata = pm.runningmedian(N[mass_mask*inner_mask*silly_mask], T[mass_mask*inner_mask*silly_mask], stat='median')
    ...
    plt.savefig(f'{plot_dir}{model}_{wind}_{snap}_NT_r200_mass.pdf', format='pdf')
    plt.close()
