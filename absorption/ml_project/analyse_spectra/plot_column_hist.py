...
if __name__ == '__main__':
    ...
    N_max = 18.
    deltaN = 0.2
    N_bins = np.arange(N_min, N_max+deltaN, deltaN)
    ...
    for l, line in enumerate(lines):
        ...
        for i in range(len(fr200)):
            ...
            ax[l][i].hist(all_N, bins=N_bins, density=True, color='k', histtype='step', label='All')
            ax[l][i].hist(all_N[sf_mask], bins=N_bins, density=True, color=cb_blue, histtype='step', label='SF')
            ax[l][i].hist(all_N[gv_mask], bins=N_bins, density=True, color=cb_green, histtype='step', label='GV')
            ax[l][i].hist(all_N[q_mask], bins=N_bins, density=True, color=cb_red, histtype='step', label='Q')

            ax[l][i].set_xlim(N_min, N_max) 
            ax[l][i].set_ylim(0, 3.)
            ...
            if i == 0:
                ...
                ax[l][i].annotate(plot_lines[l], xy=(0.75, 0.85), xycoords='axes fraction')
                if l == 0:
                    ax[l][i].legend(loc=2)

    plt.tight_layout()
    fig.subplots_adjust(wspace=0., hspace=0.)
    plt.savefig(f'{plot_dir}{model}_{wind}_{snap}_column_hist_all_fr200.png')
    plt.close()

    inner_outer = [[0.25, 0.5, 0.75], [1.0, 1.25]]

    # inner and outer cgm 
    fig, ax = plt.subplots(len(lines), 2, figsize=(6, 13), sharey='row', sharex='col')
    ...
    for l, line in enumerate(lines):
        ...
        for i in range(len(inner_outer)):
            ...
            ax[l][i].set_xlim(N_min, N_max)
            ax[l][i].set_ylim(0, 1.75)

            if l == 0:
                if i == 0:
                    ax[l][i].set_title('Inner CGM')
                elif i == 1:
                    ax[l][i].set_title('Outer CGM')
            if l == len(lines)-1:
                ax[l][i].set_xlabel(r'${\rm log }(N / {\rm cm}^{-2})$')
            if i == 0:
                ax[l][i].set_ylabel(r'${\rm Frequency}$')
    ...
    plt.tight_layout()
    fig.subplots_adjust(wspace=0., hspace=0.)
    plt.savefig(f'{plot_dir}{model}_{wind}_{snap}_column_hist.png')
    plt.show()
    plt.close()
