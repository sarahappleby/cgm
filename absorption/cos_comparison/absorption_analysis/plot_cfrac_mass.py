...
if __name__ == '__main__':
    ...
    plot_name = model+'_'+wind +'_'+background+'_rho_cfrac_mass'
    if r200_scaled:
        scale_str = '_scaled'
        plot_name += scale_str
        xlabel = r'$\rho / r_{200}$'
    ...
    cos_halos_file = basic_dir+'data/cos_halos_obs_cfrac_data_mass'+scale_str+'.h5'
    cos_halos_plot_dict = read_dict_from_h5(cos_halos_file)
    cos_dwarfs_file = basic_dir+'data/cos_dwarfs_obs_cfrac_data_mass'+scale_str+'.h5'
    cos_dwarfs_plot_dict = read_dict_from_h5(cos_dwarfs_file)
    sim_halos_file = basic_dir+'data/cos_halos_'+model+'_'+wind+'_137_'+background+'_sim_cfrac_data_mass'+scale_str+'.h5'
    sim_halos_plot_dict = read_dict_from_h5(sim_halos_file)
    sim_dwarfs_file = basic_dir+'data/cos_dwarfs_'+model+'_'+wind+'_151_'+background+'_sim_cfrac_data_mass'+scale_str+'.h5'
    ...
    leg_mass = ax[0].legend(mass_lines,mass_plot_labels, loc=4, fontsize=13, framealpha=0.)
    ...
    for i, survey in enumerate(cos_survey):
        ...
        leg_data = ax[i].legend(data_lines, [label, 'Simba'], loc=1, fontsize=13, framealpha=0.)
        ax[i].add_artist(leg_data)

        for m, mass_label in enumerate(mass_labels):

            l1, = ax[i].plot(sim_plot_dict['plot_bins'], sim_plot_dict[f'cfrac_{lines[i]}_{mass_label}'], c=colors[m], ls='-', marker='', lw=2)
            ax[i].fill_between(sim_plot_dict['plot_bins'],
                               sim_plot_dict[f'cfrac_{lines[i]}_{mass_label}'] - sim_plot_dict[f'cfrac_{lines[i]}_poisson_{mass_label}'],
                               sim_plot_dict[f'cfrac_{lines[i]}_{mass_label}'] + sim_plot_dict[f'cfrac_{lines[i]}_poisson_{mass_label}'],
                               color=colors[m], alpha=0.25)

            c1 = ax[i].errorbar(cos_plot_dict['plot_bins'], cos_plot_dict[f'cfrac_{lines[i]}_{mass_label}'], 
                                yerr=cos_plot_dict[f'cfrac_{lines[i]}_poisson_{mass_label}'], xerr=cos_plot_dict['xerr'], 
