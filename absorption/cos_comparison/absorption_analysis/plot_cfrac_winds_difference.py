...
if __name__ == '__main__':
    ...
    simba_halos_file = basic_dir+'data/cos_halos_'+model+'_s50j7k_137_'+background+'_sim_cfrac_data'+scale_str+'.h5'
    simba_halos_plot_dict = read_dict_from_h5(simba_halos_file)
    simba_dwarfs_file = basic_dir+'data/cos_dwarfs_'+model+'_s50j7k_151_'+background+'_sim_cfrac_data'+scale_str+'.h5'
    simba_dwarfs_plot_dict = read_dict_from_h5(simba_dwarfs_file)

    for j, wind in enumerate(winds):

        sim_halos_file = basic_dir+'data/cos_halos_'+model+'_'+wind+'_137_'+background+'_sim_cfrac_data.h5'
        sim_halos_plot_dict = read_dict_from_h5(sim_halos_file)
        sim_dwarfs_file = basic_dir+'data/cos_dwarfs_'+model+'_'+wind+'_151_'+background+'_sim_cfrac_data.h5'
        ...
        for i, survey in enumerate(cos_survey):
            ...
            diff = sim_plot_dict['cfrac_'+lines[i]+'_sf'] - simba_plot_dict['cfrac_'+lines[i]+'_sf']
            err = np.sqrt(sim_plot_dict['cfrac_'+lines[i]+'_poisson_sf']**2 + simba_plot_dict['cfrac_'+lines[i]+'_poisson_sf']**2)
