...
if os.path.isfile(frac_stats_file):
    ...
    fractions = {}
    with h5py.File(fracdata_dir+'omega_mass_fraction.h5', 'r') as hf:
        for p in all_phases:
            fractions[p] = hf[p][:]
...
for i, phase in enumerate(plot_phases):
    plt.errorbar(plot_bins, frac_stats[phase]['median'], yerr=frac_stats[phase]['percentile_25_75'], color=colours[i], label=phase)

plt.legend(loc=2)
plt.xlabel('M*')
plt.ylabel('Mass fraction (omega)')
plt.xlim(min_mass, max_mass)
plt.savefig(savedir+model+'_'+wind+'_'+snap+'_omega_fracs.png')
plt.clf()
