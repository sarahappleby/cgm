...
for w, wind in enumerate(winds):
    ...
    if os.path.isfile(frac_stats_file):
        ...
        frac_stats['mhalo_bins'] = get_bin_middle(np.append(mass_bins, mass_bins[-1] + dm))  

        mask = np.array([True] * len(gal_sm))
        frac_stats['all'] = get_phase_stats(gal_mhalo, gal_pos, fractions, mask, all_phases, mass_bins, boxsize, logresults=False)

        mask = gal_ssfr > quench
        frac_stats['star_forming'] = get_phase_stats(gal_mhalo, gal_pos, fractions, mask, all_phases, mass_bins, boxsize, logresults=False)

        mask = gal_ssfr < quench
        frac_stats['quenched'] = get_phase_stats(gal_mhalo, gal_pos, fractions, mask, all_phases, mass_bins, boxsize, logresults=False)

        write_phase_stats(frac_stats_file, frac_stats, all_phases, stats)

    mask = frac_stats['all']['ngals'][:] > ngals_min

    running_total = np.zeros(len(frac_stats['mhalo_bins']))
    for i, phase in enumerate(plot_phases):
        if phase == 'Dust':
            continue
        ax[w].fill_between(frac_stats['mhalo_bins'][mask], running_total[mask], running_total[mask] + frac_stats['all'][phase]['median'][mask], 
    ...
    ax[w].set_ylim(0, 1)
    ax[w].set_xlabel(r'$\textrm{log} (M_{halo} / \textrm{M}_{\odot})$')
...
plt.savefig(savedir+model+'_'+snap+'_omega_fracs_peeples_winds_mhalo.png', bbox_inches = 'tight')
plt.clf()
