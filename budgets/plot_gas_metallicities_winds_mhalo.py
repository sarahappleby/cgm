...
snap = '151'
winds = ['s50', 's50nox', 's50nojet', 's50noagn', 's50nofb']
model = 'm50n512'
boxsize = 50000.
...
for w, wind in enumerate(winds):
        ...
        z_stats_file = data_dir+model+'_'+wind+'_'+snap+'_metallicities_mhalo_stats.h5'
        ...
        if os.path.isfile(z_stats_file):
                ...
                gal_mhalo = np.array([i.halo.masses['total'].in_units('Msun') for i in sim.galaxies])[central]
                ...
                z_stats['mhalo_bins'] = get_bin_middle(np.append(mass_bins, mass_bins[-1] + dm))

                mask = np.array([True] * len(gal_mhalo))
                z_stats['all'] = get_phase_stats(gal_mhalo, gal_pos, metallicities, mask, all_phases, mass_bins, boxsize, logresults=True)

                mask = gal_ssfr > quench
                z_stats['star_forming'] = get_phase_stats(gal_mhalo, gal_pos, metallicities, mask, all_phases, mass_bins, boxsize, logresults=True)

                mask = gal_ssfr < quench
                z_stats['quenched'] = get_phase_stats(gal_mhalo, gal_pos, metallicities, mask, all_phases, mass_bins, boxsize, logresults=True)

                write_phase_stats(z_stats_file, z_stats, all_phases, stats)

        if w == 0:
            z_stats['mhalo_bins'] -= 1.5* xoffset
        elif w == 1:
            z_stats['mhalo_bins'] -= 0.5* xoffset
        elif w == 2:
            z_stats['mhalo_bins'] += 0.5* xoffset
        elif w == 3:
            z_stats['mhalo_bins'] += 1.5* xoffset
        ...
        for i, phase in enumerate(plot_phases):
            l1 = ax[i].errorbar(z_stats['mhalo_bins'][mask], z_stats['all'][phase]['median'][mask],
            ...
            if w == 0:
                ...
                ax[i].set_xlim(min_mass, z_stats['mhalo_bins'][-1]+0.5*dm)
...
plt.savefig(savedir+model+'_'+snap+'_gas_metallcities_winds_mhalo.png', bbox_inches = 'tight')
plt.clf()
