...
for w, wind in enumerate(winds):
    ...
    for i, phase in enumerate(plot_phases):
        ...
        if (wind == 's50nofb') and (i < 2) :
            diff[diff < -2.] = -2.
            l1 = ax[i].errorbar(z_stats['mhalo_bins'][mask][0], diff[mask][0], 
        ...
        if w == 0:
            ...
            ax[i].set_ylim(-1.3, .8)
            ax[i].set_xlabel(r'$\textrm{log} (M_{halo} / \textrm{M}_{\odot})$')
...
plt.savefig(savedir+model+'_'+snap+'_metallcities_difference_mhalo.png', bbox_inches = 'tight',
            metadata={'creator': 'plot_gas_metallicities_difference_mhalo.py'})
plt.clf()
