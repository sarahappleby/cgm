...
from physics import compute_path_abs, compute_path_length
...
for i, survey in enumerate(cos_survey):
    ...
    for j, w in enumerate(winds):
        ...
        ax[i].plot(plot_bins, np.log10(sim_sf_path_abs), linestyle=ls[j], c='b', )
        ax[i].plot(plot_bins, np.log10(sim_q_path_abs), linestyle=ls[j], c='r', )
    ...
    if compare:
        ...
        c1, = ax[i].plot(plot_bins, np.log10(cos_sf_path_abs), c='c', marker='o', ls='--', label=label+' SF')
        c2, = ax[i].plot(plot_bins, np.log10(cos_q_path_abs), c='m', marker='o', ls='--', label=label+' Q')
...
plt.savefig(plot_dir+model+'_winds_rho_path_abs.png') 
