...
c
	...
	integer subsub(maxregions,2),nsubsub
...
c  find regions where lines are detected above N_sigma significance
	i = ew_finder(1,ndata,subreg,nsubreg,resid)
c	do isubreg=1,nsubreg
c	  write(6,*) isubreg,vel(subreg(isubreg,1)),
c     &		vel(subreg(isubreg,2))
c	end do
c  loop until all detection regions processed
	do 35 isubreg=1,nsubreg
c  set up region for fitting
	  imin = subreg(isubreg,1)
	  imax = subreg(isubreg,2)
	  write(6,*) 'region ',isubreg,vel(imin),vel(imax)
	  region(nlines,1) = imin
	  region(nlines,2) = imax
c  keep adding lines until no significant detections within region
	  do 40 while (ew_finder(imin,imax,subsub,nsubsub,resid).ne.0)
	    nlines = nlines + 1
c  smooth data
